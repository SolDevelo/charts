...
```console
helm install my-release oci://MY-OCI-REGISTRY/cassandra
```
...
| `cluster.rackUpdateNetworkStrategy.enabled`   | Run init script for updating the rack network strategy (only when there is more than one rack)                                                                                                                    | `true`                      |
| `cluster.rackUpdateNetworkStrategy.retries`   | Number of retries for the init script                                                                                                                                                                             | `30`                        |
| `cluster.rackUpdateNetworkStrategy.sleepTime` | Sleep time between retries                                                                                                                                                                                        | `20`                        |
...
| `dynamicSeedDiscovery.fips.openssl`           | Configure OpenSSL FIPS mode: '', 'restricted', 'relaxed', 'off'. If empty (""), 'global.defaultFips' would be used                                                                                                | `""`                        |
...
| `tls.fips.java`                 | Configure Java FIPS mode: '', 'restricted', 'relaxed', 'off'. If empty (""), 'global.defaultFips' would be used                                                                                                                        | `""`    |
