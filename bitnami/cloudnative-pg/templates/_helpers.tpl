...
{{- if or .Values.pluginBarmanCloud.tls.autoGenerated.enabled (and (not (empty .Values.pluginBarmanCloud.tls.client.cert)) (not (empty .Values.pluginBarmanCloud.tls.client.key))) -}}
    {{- printf "%s-client-crt" (include "cloudnative-pg.plugin-barman-cloud.fullname" .) -}}
{{- else -}}
    {{- required "An existing secret name must be provided with TLS certs for Plugin Barman Cloud clients if cert and key are not provided!" (tpl .Values.pluginBarmanCloud.tls.client.existingSecret .) -}}
{{- end -}}
{{- end -}}
...
{{- if or .Values.pluginBarmanCloud.tls.autoGenerated.enabled (and (not (empty .Values.pluginBarmanCloud.tls.server.cert)) (not (empty .Values.pluginBarmanCloud.tls.server.key))) -}}
    {{- printf "%s-server-crt" (include "cloudnative-pg.plugin-barman-cloud.fullname" .) -}}
{{- else -}}
    {{- required "An existing secret name must be provided with TLS certs for Plugin Barman Cloud servers if cert and key are not provided!" (tpl .Values.pluginBarmanCloud.tls.server.existingSecret .) -}}
{{- end -}}
{{- end -}}
