...
{{- if or .Values.server.tls.autoGenerated.enabled (and (not (empty .Values.server.tls.cert)) (not (empty .Values.server.tls.key))) -}}
    {{- printf "%s-crt" (include "vault.server.fullname" .) -}}
{{- else -}}
    {{- required "An existing secret name must be provided with TLS certs for Vault server if cert and key are not provided!" (tpl .Values.server.tls.existingSecret .) -}}
{{- end -}}
{{- end -}}
