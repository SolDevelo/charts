...
```console
helm install my-release oci://MY-OCI-REGISTRY/grafana-alloy
```
...
- Using Helm capabilities. Enable this feature by setting `alloy.tls.autoGenerated.enabled` to `true` and `alloy.tls.autoGenerated.engine` to `helm`.
- Relying on CertManager (please note it's required to have CertManager installed in your K8s cluster). Enable this feature by setting `alloy.tls.autoGenerated.enabled` to `true` and `alloy.tls.autoGenerated.engine` to `cert-manager`. Please note it's supported to use an existing Issuer/ClusterIssuer for issuing the TLS certificates by setting the `alloy.tls.autoGenerated.certManager.existingIssuer` and `alloy.tls.autoGenerated.certManager.existingIssuerKind` parameters.
...
| `alloy.tls.enabled`                                       | If disabled, the alloy container will be configured without HTTP certificates                                                                                                                                                        | `false`                         |
| `alloy.tls.existingSecret`                                | Name of a Secret containing the proxy TLS certificates                                                                                                                                                                               | `""`                            |
| `alloy.tls.certFilename`                                  | The secret key from the existing Secret if 'cert' key different from the default (tls.crt)                                                                                                                                           | `tls.crt`                       |
| `alloy.tls.certKeyFilename`                               | The secret key from the existing Secret if 'key' key different from the default (tls.key)                                                                                                                                            | `tls.key`                       |
| `alloy.tls.certCAFilename`                                | The secret key from the existing Secret if 'ca' key different from the default (tls.crt)                                                                                                                                             | `""`                            |
| `alloy.tls.ca`                                            | CA certificate for TLS. Ignored if `alloy.tls.existingSecret` is set                                                                                                                                                                 | `""`                            |
| `alloy.tls.cert`                                          | TLS certificate for Grafana Alloy. Ignored if `alloy.tls.existingSecret` is set                                                                                                                                                      | `""`                            |
| `alloy.tls.key`                                           | TLS key for Grafana Alloy. Ignored if `alloy.tls.existingSecret` is set                                                                                                                                                              | `""`                            |
| `alloy.tls.autoGenerated.enabled`                         | Enable automatic generation of TLS certificates                                                                                                                                                                                      | `true`                          |
| `alloy.tls.autoGenerated.engine`                          | Mechanism to generate the certificates (allowed values: helm, cert-manager)                                                                                                                                                          | `helm`                          |
| `alloy.tls.autoGenerated.extraSANs`                       | Extra Subject Alternative Names (SANs) for generated certificates                                                                                                                                                                    | `[]`                            |
| `alloy.tls.autoGenerated.loopback`                        | Add loopback SANs (localhost and 127.0.0.1) to generated certificates                                                                                                                                                                | `false`                         |
| `alloy.tls.autoGenerated.certManager.existingIssuer`      | The name of an existing Issuer to use for generating the certificates (only for `cert-manager` engine)                                                                                                                               | `""`                            |
| `alloy.tls.autoGenerated.certManager.existingIssuerKind`  | Existing Issuer kind, defaults to Issuer (only for `cert-manager` engine)                                                                                                                                                            | `""`                            |
| `alloy.tls.autoGenerated.certManager.keyAlgorithm`        | Key algorithm for the certificates (only for `cert-manager` engine)                                                                                                                                                                  | `RSA`                           |
| `alloy.tls.autoGenerated.certManager.keySize`             | Key size for the certificates (only for `cert-manager` engine)                                                                                                                                                                       | `2048`                          |
| `alloy.tls.autoGenerated.certManager.duration`            | Duration for the certificates (only for `cert-manager` engine)                                                                                                                                                                       | `2160h`                         |
| `alloy.tls.autoGenerated.certManager.renewBefore`         | Renewal period for the certificates (only for `cert-manager` engine)                                                                                                                                                                 | `360h`                          |
...
| `alloy.fips.openssl`                                      | Configure OpenSSL FIPS mode: '', 'restricted', 'relaxed', 'off'. If empty (""), 'global.defaultFips' would be used                                                                                                                   | `""`                            |
| `alloy.fips.golang`                                       | Configure Golang FIPS mode: '', 'restricted', 'relaxed', 'off'. If empty (""), 'global.defaultFips' would be used                                                                                                                    | `restricted`                    |
...
| `configReloader.fips.openssl`                                      | Configure OpenSSL FIPS mode: '', 'restricted', 'relaxed', 'off'. If empty (""), 'global.defaultFips' would be used                                                                                                                                        | `""`                                       |
| `configReloader.fips.golang`                                       | Configure Golang FIPS mode: '', 'restricted', 'relaxed', 'off'. If empty (""), 'global.defaultFips' would be used                                                                                                                                         | `restricted`                               |
