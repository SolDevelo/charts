...
```console
helm install my-release oci://MY-OCI-REGISTRY/keydb
```
...
| `tls.master.existingSecret`                        | The name of an existing Secret containing the TLS certificates for KeyDB master nodes                  | `""`          |
| `tls.master.certFilename`                          | The secret key from the existing Secret if 'cert' key is different from the default (tls.crt)          | `tls.crt`     |
| `tls.master.certKeyFilename`                       | The secret key from the existing Secret if 'key' key is different from the default (tls.key)           | `tls.key`     |
...
| `tls.replica.existingSecret`                       | The name of an existing Secret containing the TLS certificates for KeyDB replica nodes                 | `""`          |
| `tls.replica.certFilename`                         | The secret key from the existing Secret if 'cert' key is different from the default (tls.crt)          | `tls.crt`     |
| `tls.replica.certKeyFilename`                      | The secret key from the existing Secret if 'key' key is different from the default (tls.key)           | `tls.key`     |
