...
At Bitnami, we are committed to ensure the quality of the assets we deliver, and as such, tests play a fundamental role in this repository.
...
Before writing any test scenario, understand the primary purpose of the Chart and its components. As Charts are usually composed of several different components, it is also essential to test their integrations and the Chart as a whole. As a general guideline, testing a chart can be reduced to:
...
1. Why was `moodleUsername` included?
    ...
    The default value for `moodleUsername` is `user` (you can check in [values.yaml](https://github.com/bitnami/charts-private/blob/main/bitnami/moodle/values.yaml)). Following the strategy, the default value was changed to see if the Chart can correctly pick it up. This is later checked [in one of the tests](https://github.com/bitnami/charts-private/blob/main/.vib/moodle/cypress/cypress/support/commands.js#L19).
...
* [ ] Apply the following code style:
  ...
  * Lines over 80 columns long should preferably be divided into sensible chunks
...
2. Download the [GOSS binary for Linux](https://github.com/goss-org/goss/releases/)
3. Copy the binary and test files to the target pod where it should be executed
    ...
    $ kubectl cp ./goss-linux-amd64 nginx-5fbc8786f-95rpl:/tmp/goss
...
4. Grant execution permissions to the binary and launch the tests
    ...
    ```console
    $ kubectl exec -it nginx-5fbc8786f-95rpl -- chmod +x /tmp/goss
    $ kubectl exec -it nginx-5fbc8786f-95rpl -- /tmp/goss --gossfile /tmp/goss.yaml --vars /tmp/runtime-parameters.yaml validate
    .........
...
[Kong](https://github.com/bitnami/charts-private/blob/main/.vib/kong/goss/goss.yaml) or [MetalLB](https://github.com/bitnami/charts-private/blob/main/.vib/metallb/goss/goss.yaml) are two good examples of tests implementing some of the above.
